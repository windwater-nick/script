# ====================================================================
#  Test-7zArchive.ps1 - 7z 格式專屬解密子腳本 v2.5 (支援自動匯出 Log 紀錄檔)
# ====================================================================
...
if (-not $sevenZipExe -or -not (Test-Path $sevenZipExe)) {
    Write-Host "[ERROR] 未檢測到可用的 7-Zip (7z.exe) 解密引擎！" -ForegroundColor Red
...
$engineName  = Split-Path $sevenZipExe -Leaf
...
try {
    while ($true) {
        ...
        for ($i = $runningJobs.Count - 1; $i -ge 0; $i--) {
            ...
            if ($job.AsyncResult.IsCompleted) {
                ...
                if (($job.EndIndex - $sharedState.LastCheckpoint) -ge 1000 -or ([System.DateTime]::Now - $lastSaveTime).TotalSeconds -ge 10) {
                    ...
                    Save-AtomicCheckpoint -RootDir $PSScriptRoot\.. -ArchiveType "7z" -Data $cpData
        ...
        $engineStr = "7-Zip ($engineName)".PadRight(25)

        [System.Console]::SetCursorPosition(0, 0)
        Write-Host "====================================================================" -ForegroundColor Cyan
        Write-Host "                       解密測試進行中 (7z)                        " -ForegroundColor Cyan
        Write-Host "====================================================================" -ForegroundColor Cyan
    ...
    Clear-Host
    Write-Host "====================================================================" -ForegroundColor Cyan
    Write-Host "                       解密測試結果結算 (7z)                       " -ForegroundColor Cyan
    Write-Host "====================================================================" -ForegroundColor Cyan
    ...
    if ($sharedState.Found) {
        ...
        Clear-CheckpointFiles -RootDir $PSScriptRoot\.. -ArchiveType "7z"
    }
    ...
    try {
        ...
        $logFileName = "Log_7z_$($startTime.ToString('yyyyMMdd_HHmmss')).log"
